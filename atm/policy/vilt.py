...
class BCViLTPolicy(nn.Module):
    ...
    def forward(self, obs, track_obs, track, task_emb, extra_states):
        ...
        return dist # action distribution?
    ...
    def forward_vis(self, obs, track_obs, track, task_emb, extra_states, action):
        ...
        Args:
            obs: b v t c h w
            track_obs: b v t tt_fs c h w (images from hdf5 file)
            track: b v t track_len n 2 (tracks from cotracker)
            task_emb: b emb_size
        Returns:
        """
        ...
        for view in range(self.num_views):
            gt_track = track[:1, view]  # (1 tl n d) tracks are sampled close to grid points (sample nearest to grid point)
            ...
            _, ret_dict = self.track.forward_vis(track_obs[:1, view, 0, :, ...], grid_track[:1, view], task_emb[:1], p_img=0) # process grid and images in track transformer (inference)
